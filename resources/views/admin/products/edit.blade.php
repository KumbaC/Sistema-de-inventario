<x-app-layout>
    ...
    <x-slot name="header">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
              <x-jet-nav-link href="{{route('admin.index')}}">  {{ __('Administrador') }} </x-jet-nav-link>
               <x-jet-nav-link :active="request()->routeIs('admin.products.edit')" href="{{route('admin.products.index')}}" class="ml-8">{{ __('Productos') }}</x-jet-nav-link>
               @can('admin.users.index') <x-jet-nav-link  href="{{route('admin.users.index')}}" class="ml-8">{{ __('Usuarios') }}</x-jet-nav-link>@endcan
               @can('admin.categories.index')   <x-jet-nav-link  href="{{route('admin.categories.index')}}" class="ml-8">{{ __('Categorias') }}</x-jet-nav-link> @endcan
            </h2>
        </x-slot>
...
<x-slot name="logo">
    ...
    {!! Form::Submit('Actualizar producto', ['class' => 'py-2 px-4  bg-blue-600 hover:bg-green-700 focus:ring-purple-500 focus:ring-offset-purple-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg ']) !!}
    </div>
